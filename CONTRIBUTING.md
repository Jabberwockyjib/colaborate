...
```bash
git clone https://github.com/NeosiaNexus/Colaborate.git
cd Colaborate
bun install
```
...
|---------|-----|--------|-------------|
| `@colaborate/core` | private | — | Shared types, schema, store errors, helpers, conformance tests |
| `@colaborate/widget` | published | Browser | Feedback widget (Shadow DOM, closed). Accepts `store` for client-side mode |
| `@colaborate/adapter-prisma` | published | Node | Prisma database adapter |
| `@colaborate/adapter-memory` | published | Any | In-memory adapter (testing, demos, serverless) |
| `@colaborate/adapter-localstorage` | published | Browser | localStorage adapter (demos, prototyping) |
| `@colaborate/cli` | published | Node | CLI tool (`colaborate init/sync/status/doctor`) |

- **Core** is an Internal Package — it exports raw TypeScript (no build step). Consumers bundle it via `noExternal: ["@colaborate/core"]` in their tsup config.
...
To add a new package to the monorepo (e.g. `@colaborate/adapter-drizzle`):
...
```jsonc
{
  "name": "@colaborate/adapter-drizzle",
  "version": "0.1.0",
  "type": "module",
  "exports": {
    ".": {
  ...
  "repository": {
    "type": "git",
    "url": "git+https://github.com/NeosiaNexus/Colaborate.git",
    "directory": "packages/adapter-drizzle"
  },
  // If you import from @colaborate/core:
  "devDependencies": {
...
> **Important:** `@colaborate/core` must be a `devDependency`, never a `dependency` — it is bundled at build time and not published to npm.
...
export default defineConfig({
  ...
  sourcemap: true,
  clean: true,
  noExternal: ["@colaborate/core"],  // bundle core (not published)
});
```
...
Adapters implement the `ColaborateStore` interface from `@colaborate/core`. To create a new one (e.g. `adapter-drizzle`):

1. Copy `packages/adapter-memory/` as a starting point (simplest adapter)
2. Implement the 6 methods of `ColaborateStore`:
...
// Runs 22 conformance tests covering the full ColaborateStore contract
testColaborateStore(() => new MyStore(testConfig))
...
```ts
export { StoreNotFoundError, StoreDuplicateError } from '@colaborate/core'
```

5. Use `flattenAnnotation()` from `@colaborate/core` if your adapter handles HTTP payloads.
