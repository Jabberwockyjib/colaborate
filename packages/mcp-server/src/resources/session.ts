...
export function register(server: McpServer, ctx: ServerContext): void {
  server.registerResource(
    ...
    new ResourceTemplate(URI_TEMPLATE, { list: undefined }),
    {
      ...
      description:
        "Full session bundle — session record + linked feedbacks (with annotations). `screenshots` is always empty until Phase 4. One-shot reads only; no subscriptions in v0.",
      mimeType: "application/json",
    },
