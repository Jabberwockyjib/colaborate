...
describe("syncCommand", () => {
  ...
  beforeEach(() => {
    tmpDir = mkdtempSync(join(tmpdir(), "colaborate-sync-test-"));
  ...
  it("uses the specified --schema path for syncing", () => {
    ...
    expect(logSuccessSpy).toHaveBeenCalledWith(expect.stringContaining("ColaborateFeedback"));
  });
  ...
  it("adds both models to an empty schema", () => {
    ...
    expect(logSuccessSpy).toHaveBeenCalledWith(expect.stringContaining("ColaborateFeedback"));
    expect(logSuccessSpy).toHaveBeenCalledWith(expect.stringContaining("ColaborateAnnotation"));
    expect(exitSpy).not.toHaveBeenCalled();
  ...
  it("reports field-level changes on a partial schema", () => {
    ...
    // ColaborateAnnotation is new, should appear in model creation log
    expect(logSuccessSpy).toHaveBeenCalledWith(expect.stringContaining("ColaborateAnnotation"));
