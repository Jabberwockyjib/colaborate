...
/** A valid Prisma schema with both Colaborate models — complete and up-to-date. */
const FULL_SCHEMA = `
datasource db {
  provider = "postgresql"
...
model ColaborateAnnotation {
  id               String           @id @default(cuid())
  feedbackId       String
  feedback         ColaborateFeedback @relation(fields: [feedbackId], references: [id], onDelete: Cascade)
...
/** Schema missing ColaborateAnnotation entirely and ColaborateFeedback is partial. */
...
function createApiRoute(dir: string): void {
  const routeDir = join(dir, "app", "api", "colaborate");
...
function createWidgetUsage(dir: string): void {
  ...
  writeFileSync(
    ...
    'import { initColaborate } from "@colaborate/widget";\ninitColaborate({ endpoint: "/api/colaborate", projectName: "test" });',
  );
}
...
describe("statusCommand", () => {
  ...
  beforeEach(() => {
    tmpDir = mkdtempSync(join(tmpdir(), "colaborate-status-test-"));
  ...
  describe("Prisma schema detection", () => {
    ...
    it("reports success when schema is found and up-to-date", () => {
      ...
      createPackageJson(tmpDir, { "@colaborate/widget": "^1.0.0" });
  ...
  describe("API route detection", () => {
    it("reports success when API route exists at app/api/colaborate/route.ts", () => {
      createPackageJson(tmpDir);
      createApiRoute(tmpDir);
    ...
    it("reports success when API route exists at src/app/api/colaborate/route.ts", () => {
      createPackageJson(tmpDir);
      const routeDir = join(tmpDir, "src", "app", "api", "colaborate");
      mkdirSync(routeDir, { recursive: true });
      writeFileSync(join(routeDir, "route.ts"), "export const GET = () => {};");
  ...
  describe("Package detection", () => {
    it("reports success when @colaborate/widget is in dependencies", () => {
      createPackageJson(tmpDir, { "@colaborate/widget": "^1.0.0" });

      statusCommand({});

      const successes = allMessages(logSuccessSpy);
      expect(successes.some((m) => m.includes("@colaborate/widget"))).toBe(true);
    });

    it("reports success when @colaborate/widget is in devDependencies", () => {
      createPackageJson(tmpDir, {}, { "@colaborate/widget": "^1.0.0" });
    ...
    it("reports error when @colaborate/widget is not in any dependencies", () => {
      createPackageJson(tmpDir, { "some-other-package": "^1.0.0" });

      statusCommand({});

      const errors = allMessages(logErrorSpy);
      expect(errors.some((m) => m.includes("@colaborate/widget"))).toBe(true);
    });
  ...
  describe("Widget integration detection", () => {
    it("reports success when initColaborate is found in source files", () => {
      createPackageJson(tmpDir, { "@colaborate/widget": "^1.0.0" });
    ...
    it("reports warning when initColaborate is not found in source files", () => {
      createPackageJson(tmpDir, { "@colaborate/widget": "^1.0.0" });
