...
describe("initCommand", () => {
  ...
  beforeEach(() => {
    tmpDir = mkdtempSync(join(tmpdir(), "colaborate-init-test-"));
  ...
  describe("schema found + sync", () => {
    it("syncs new models when confirmed", async () => {
      ...
      expect(p.log.success).toHaveBeenCalledWith(expect.stringContaining("ColaborateFeedback"));
      expect(p.log.success).toHaveBeenCalledWith(expect.stringContaining("ColaborateAnnotation"));
    });
...
`,
      ...
      // ColaborateAnnotation model should be added
      expect(successes.some((m) => m.includes("ColaborateAnnotation"))).toBe(true);
    ...
    it("does not sync when user declines", async () => {
      ...
      // Schema should not have been modified (no ColaborateFeedback model)
      const successes = allMessages(vi.mocked(p.log.success));
  ...
  describe("route generation", () => {
    it("shows success when route is created", async () => {
      ...
      expect(existsSync(join(tmpDir, "app", "api", "colaborate", "route.ts"))).toBe(true);
    });
    ...
    it("does not generate route when user declines", async () => {
      ...
      expect(existsSync(join(tmpDir, "app", "api", "colaborate", "route.ts"))).toBe(false);
    });
  });
  ...
  describe("full flow", () => {
    it("runs schema sync + route generation when both confirmed", async () => {
      ...
      const successes = allMessages(vi.mocked(p.log.success));
      expect(successes.some((m) => m.includes("ColaborateFeedback"))).toBe(true);
  ...
  describe("UI framing", () => {
    it("calls intro and outro", async () => {
      ...
      expect(p.intro).toHaveBeenCalledWith("colaborate — Setup");
