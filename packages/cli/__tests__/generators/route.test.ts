...
describe("generateRoute", () => {
  ...
  beforeEach(() => {
    tmpDir = mkdtempSync(join(tmpdir(), "colaborate-route-test-"));
  });
  ...
  it("creates route in app/api/colaborate/route.ts when app/ exists", () => {
    mkdirSync(join(tmpDir, "app"), { recursive: true });

    const result = generateRoute(tmpDir);

    expect(result.created).toBe(true);
    expect(result.path).toBe(join(tmpDir, "app", "api", "colaborate", "route.ts"));
    expect(existsSync(result.path)).toBe(true);
  });

  it("creates route in src/app/api/colaborate/route.ts when src/app/ exists", () => {
    mkdirSync(join(tmpDir, "src", "app"), { recursive: true });

    const result = generateRoute(tmpDir);

    expect(result.created).toBe(true);
    expect(result.path).toBe(join(tmpDir, "src", "app", "api", "colaborate", "route.ts"));
    expect(existsSync(result.path)).toBe(true);
  });
  ...
  it("generates valid TypeScript with correct imports and exports", () => {
    ...
    expect(content).toContain('import { createColaborateHandler } from "@colaborate/adapter-prisma"');
    expect(content).toContain('import { prisma } from "@/lib/prisma"');
    expect(content).toContain("export const { GET, POST, PATCH, DELETE, OPTIONS } = createColaborateHandler({");
    expect(content).toContain("prisma,");
    expect(content).toContain("// apiKey: process.env.COLABORATE_API_KEY,");
  ...
  it("throws descriptive error message on EACCES permission error", () => {
    ...
    mkdirSync(join(tmpDir, "app", "api", "colaborate"), { recursive: true });

    // Make the target directory read-only to trigger EACCES on write
    const { chmodSync } = require("node:fs");
    const targetDir = join(tmpDir, "app", "api", "colaborate");
