...
export async function doctorCommand(options: { url?: string; endpoint?: string }): Promise<void> {
  p.intro("colaborate — Network diagnostics");
  ...
  const endpoint =
    ...
    (await p.text({
      ...
      placeholder: "/api/colaborate",
      defaultValue: "/api/colaborate",
    }));
  ...
  const projectName = "__colaborate_health_check__";
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (data && typeof data.total === "number") {
        ...
        p.log.warn("Unexpected response — make sure the endpoint uses createColaborateHandler()");
      }
    } else {
