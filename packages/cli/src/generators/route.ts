...
const ROUTE_TEMPLATE = `import { createColaborateHandler } from "@colaborate/adapter-prisma";
 ...
 *
 * Creates `app/api/colaborate/route.ts` with the handler setup.
...
export function generateRoute(basePath: string = process.cwd()): { created: boolean; path: string } {
  ...
  const routePath = join(appDir, "api", "colaborate", "route.ts");
