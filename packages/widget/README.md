[![npm version](https://img.shields.io/npm/v/@colaborate/widget)](https://www.npmjs.com/package/@colaborate/widget)
...
You also need a server-side adapter — see [`@colaborate/adapter-prisma`](https://www.npmjs.com/package/@colaborate/adapter-prisma).
...
Feedback persists in `localStorage` — no server, no database. Perfect for demos and prototyping. See [`@colaborate/adapter-localstorage`](https://www.npmjs.com/package/@colaborate/adapter-localstorage) and [`@colaborate/adapter-memory`](https://www.npmjs.com/package/@colaborate/adapter-memory).
...
All configuration options for `initColaborate()`:

| Option | Type | Default | Description |
|--------|------|---------|-------------|
| `endpoint` | `string` | — | Your API route (e.g. `/api/colaborate`). Required unless `store` is provided |
| `store` | `ColaborateStore` | — | Direct store for client-side mode. When set, bypasses HTTP |
...
`initColaborate()` returns a `ColaborateInstance` with the following methods:

```ts
const widget = initColaborate({ ... })
