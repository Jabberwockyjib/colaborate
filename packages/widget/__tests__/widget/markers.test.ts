...
import type { AnnotationResponse, FeedbackResponse } from "@colaborate/core";
...
function makeAnnotation(overrides: Partial<AnnotationResponse> = {}): AnnotationResponse {
  return {
    ...
    neighborText: "",
    shape: "rectangle",
    geometry: JSON.stringify({ shape: "rectangle", x: 0.1, y: 0.1, w: 0.5, h: 0.5 }),
    scrollX: 0,
    scrollY: 0,
    viewportW: 1920,
    viewportH: 1080,
...
describe("MarkerManager", () => {
  ...
  afterEach(() => {
    ...
    for (const el of document.querySelectorAll("#colaborate-markers")) {
      el.remove();
    }
    if (mockState.element) {
  ...
  describe("render", () => {
    it("creates a container element with id colaborate-markers", () => {
