...
describe("ApiClient", () => {
  ...
  const endpoint = "http://localhost/api/colaborate";
...
describe("flushRetryQueue", () => {
  ...
  it("retries queued items and removes on success", async () => {
    ...
    expect(fetch).toHaveBeenCalledTimes(1);
    expect(localStorage.removeItem).toHaveBeenCalledWith("colaborate_retry_queue");
  });
  ...
  it("keeps failed items in queue after partial failure", async () => {
    ...
    expect(localStorage.setItem).toHaveBeenCalledWith("colaborate_retry_queue", expect.stringContaining("item2"));
  });
  ...
  it("handles fetch throwing (network error) for queued items", async () => {
    ...
    expect(localStorage.setItem).toHaveBeenCalledWith("colaborate_retry_queue", expect.stringContaining("fail"));
  });
});
...
describe("queueForRetry (via sendFeedback)", () => {
  ...
  it("queues payload to localStorage when sendFeedback fails", async () => {
    ...
    expect(localStorage.setItem).toHaveBeenCalledWith("colaborate_retry_queue", expect.stringContaining("queued"));
  });
