[![npm version](https://img.shields.io/npm/v/@colaborate/adapter-prisma)](https://www.npmjs.com/package/@colaborate/adapter-prisma)
...
Server-side Prisma adapter for [Colaborate](https://github.com/NeosiaNexus/Colaborate) — handles API request validation and database persistence.
...
```bash
npm install @colaborate/adapter-prisma
```
...
```ts
// app/api/colaborate/route.ts (Next.js App Router)
...
export const { GET, POST, PATCH, DELETE, OPTIONS } = createColaborateHandler({
  prisma,
  apiKey: process.env.COLABORATE_API_KEY,
