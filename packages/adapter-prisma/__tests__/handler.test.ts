...
describe("createColaborateHandler", () => {
  let prisma: ReturnType<typeof mockPrisma>;
  let handler: ReturnType<typeof createColaborateHandler>;

  beforeEach(() => {
    prisma = mockPrisma();
    handler = createColaborateHandler({ prisma });
  });
  ...
  describe("POST", () => {
    it("creates a feedback with valid payload", async () => {
      ...
      expect(prisma.colaborateFeedback.create).toHaveBeenCalledOnce();
    });
    ...
    it("handles duplicate clientId gracefully", async () => {
      prisma.colaborateFeedback.create.mockRejectedValue({ code: "P2002" });
      prisma.colaborateFeedback.findUnique.mockResolvedValue({ id: "fb-1", ...validPayloadNoAnnotations });
      const req = new Request("http://localhost/api/colaborate", {
        method: "POST",
        body: JSON.stringify(validPayloadNoAnnotations),
      });
    ...
    it("returns 500 on unexpected DB error", async () => {
      ...
      prisma.colaborateFeedback.create.mockRejectedValue(new Error("DB down"));
      const req = new Request("http://localhost/api/colaborate", {
        method: "POST",
        body: JSON.stringify(validPayloadNoAnnotations),
      });
    ...
    it("maps annotation anchor fields to Prisma create", async () => {
      ...
      expect(prisma.colaborateFeedback.create).toHaveBeenCalledOnce();
      const createArg = prisma.colaborateFeedback.create.mock.calls[0][0] as {
  ...
  describe("GET", () => {
    it("returns feedbacks for a project", async () => {
      prisma.colaborateFeedback.findMany.mockResolvedValue([]);
      prisma.colaborateFeedback.count.mockResolvedValue(0);
      const req = new Request("http://localhost/api/colaborate?projectName=test");
    ...
    it("rejects limit > 100 via Zod validation", async () => {
      const req = new Request("http://localhost/api/colaborate?projectName=test&limit=999");
      const res = await handler.GET(req);
      expect(res.status).toBe(400);
    });
    ...
    it("applies type and status filters", async () => {
      prisma.colaborateFeedback.findMany.mockResolvedValue([]);
      prisma.colaborateFeedback.count.mockResolvedValue(0);
      const req = new Request("http://localhost/api/colaborate?projectName=test&type=bug&status=open");
      await handler.GET(req);
      const callArgs = prisma.colaborateFeedback.findMany.mock.calls[0][0] as { where: Record<string, unknown> };
  ...
  describe("PATCH", () => {
    it("resolves a feedback", async () => {
      prisma.colaborateFeedback.update.mockResolvedValue({
        id: "fb-1",
      ...
      expect(res.status).toBe(200);
      const updateArgs = prisma.colaborateFeedback.update.mock.calls[0][0] as { data: Record<string, unknown> };
  ...
  describe("DELETE", () => {
    it("deletes a single feedback by id", async () => {
      prisma.colaborateFeedback.findUnique.mockResolvedValue({ id: "fb-1", projectName: "test-project" });
      const req = new Request("http://localhost/api/colaborate", {
        method: "DELETE",
      ...
      expect(res.status).toBe(200);
      expect(prisma.colaborateFeedback.delete).toHaveBeenCalledWith({ where: { id: "fb-1" } });
    });
    ...
    it("deletes all feedbacks for a project", async () => {
      ...
      const res = await handler.DELETE(req);
      expect(res.status).toBe(200);
      expect(prisma.colaborateFeedback.deleteMany).toHaveBeenCalledWith({ where: { projectName: "test" } });
    });
    ...
    it("returns 404 when feedback not found (P2025)", async () => {
      prisma.colaborateFeedback.findUnique.mockResolvedValue({ id: "nonexistent", projectName: "test-project" });
      prisma.colaborateFeedback.delete.mockRejectedValue({ code: "P2025" });
      const req = new Request("http://localhost/api/colaborate", {
        method: "DELETE",
    ...
    it("returns 500 on unexpected DB error", async () => {
      const consoleSpy = vi.spyOn(console, "error").mockImplementation(() => {});
      prisma.colaborateFeedback.findUnique.mockResolvedValue({ id: "fb-1", projectName: "test-project" });
      prisma.colaborateFeedback.delete.mockRejectedValue(new Error("DB down"));
      const req = new Request("http://localhost/api/colaborate", {
        method: "DELETE",
        body: JSON.stringify({ id: "fb-1", projectName: "test-project" }),
      });
